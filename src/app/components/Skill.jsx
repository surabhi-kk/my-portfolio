...
function Skill() {
  ...
  return (
    ...
    <div className='bg-gray-700 items-center text-center justify-center py-10 px-10 h-[100vh]'>
      <span className='text-5xl text-bold text-white'>Technical </span>
      <span className='text-5xl text-bold text-blue-500'>Skills</span>
      <div className="grid grid-cols-5 gap-y-12 gap-x-2 mt-10 ml-16 mr-4">
  {skill.map((s) => (
    <div
      key={s.name}
      className="w-44 h-44  shadow-white shadow-md flex flex-col items-center justify-center gap-6 py-6 text-white shadow-lg rounded-2xl p-6 w-40 text-center hover:bg-blue-500 hover:scale-105 transition-transform duration-300"
    >
      <img
        src={s.image}
        alt={s.name}
